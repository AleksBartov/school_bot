...
async function sendTestQuestion(ctx, question) {
  ...
  const pollMessage = await ctx.replyWithPoll(questionText, options, {
    is_anonymous: false,
    type: "regular",
  });
...
bot.start(async (ctx) => {
  ...
  await ctx.reply(
    welcomeText,
    Markup.keyboard([["🙋 Я буду крестным", "🎯 Хочу креститься сам"]]).resize()
  );
...
bot.on("poll_answer", async (ctx) => {
  ...
  if (nextQuestion) {
    ...
    const nextQuestionText = `Вопрос ${testService.getProgress(
      nextQuestion.id
...
async function completeTestForUser(userId) {
  ...
  const levelMessages = {
    beginner:
      "Спасибо за ответы! Я вижу, что вы только начинаете знакомство с верой. Я буду давать вам простые и понятные объяснения.",
    basic:
      "Спасибо за ответы! У вас есть базовые знания. Я буду дополнять их и помогать углубить понимание.",
    churched:
      "Спасибо за ответы! Вижу, что вы уже воцерковлены. Смогу давать вам более глубокие материалы и ссылки на источники.",
  };
...
bot.hears("📋 Можно ли мне быть крестным?", async (ctx) => {
  ...
  switch (user.knowledgeLevel) {
    case "beginner":
      response =
        "Здесь будет простое объяснение для начинающих о том, кто может быть крестным...";
      break;
    case "basic":
      response =
        "Здесь будет базовое объяснение с основными требованиями к крестным...";
      break;
    case "churched":
      response =
        "Здесь будет подробное объяснение с ссылками на каноны и церковные правила...";
      break;
    default:
